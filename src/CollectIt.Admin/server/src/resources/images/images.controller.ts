...
export class ImagesController {
    ...
    @Get('')
    async getAllImagesPaged(@Query('page_number', new ParseIntPipe()) pageNumber: number,
        ...
        const images = await this.imageService.getAllImagesPagedOrderedAsync(pageNumber, pageSize);
    ...
    @Get(':imageId')
    async findImageById(@Param('imageId', new ParseIntPipe()) imageId: number): Promise<ReadImageDto> {
        try {
    ...
    @Post(':imageId/name')
    @AuthorizeAdmin()
    async changeImageName(@Param('imageId', new ParseIntPipe()) imageId: number, @Body('name') name: string) {
        try {
            if (name?.length < 6) {
    ...
    @Post(':imageId/tags')
    @AuthorizeAdmin()
    async changeImageTags(@Param('imageId', new ParseIntPipe()) imageId: number,
